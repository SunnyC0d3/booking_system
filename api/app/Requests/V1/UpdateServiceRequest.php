<?php

namespace App\Requests\V1;

class UpdateServiceRequest extends BaseFormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     */
    public function authorize(): bool
    {
        $service = $this->route('service');
        $user = $this->user();

        // Check if user has permission to edit services
        if (!$user->hasPermission('edit_services')) {
            return false;
        }

        // Check if user can edit this specific service
        if (!$user->hasPermission('edit_all_services') && $service->vendor_id !== $user->id) {
            return false;
        }

        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     */
    public function rules(): array
    {
        return [
            'name' => 'sometimes|string|max:255',
            'description' => 'sometimes|string|max:5000',
            'short_description' => 'nullable|string|max:500',
            'category' => 'sometimes|string|max:100',
            'base_price' => 'sometimes|integer|min:0|max:99999999', // in pence
            'duration_minutes' => 'sometimes|integer|min:15|max:480', // 15 minutes to 8 hours
            'vendor_id' => 'sometimes|exists:users,id',
            'is_active' => 'sometimes|boolean',
            'is_bookable' => 'sometimes|boolean',
            'requires_consultation' => 'sometimes|boolean',
            'consultation_duration_minutes' => 'nullable|integer|min:15|max:120',
            'min_advance_booking_hours' => 'nullable|integer|min:1|max:8760',
            'max_advance_booking_days' => 'nullable|integer|min:1|max:365',
            'cancellation_policy' => 'nullable|string|max:1000',
            'terms_and_conditions' => 'nullable|string|max:2000',
            'preparation_notes' => 'nullable|string|max:1000',
            'sort_order' => 'sometimes|integer|min:0|max:9999',
            'metadata' => 'sometimes|array',
            'images' => 'sometimes|array|max:10',
            'images.*' => 'image|mimes:jpeg,jpg,png,webp|max:5120',
            'replace_images' => 'sometimes|boolean', // Whether to replace existing images
        ];
    }

    /**
     * Get custom messages for validator errors.
     */
    public function messages(): array
    {
        return [
            'name.max' => 'Service name cannot exceed 255 characters.',
            'description.max' => 'Description cannot exceed 5000 characters.',
            'base_price.integer' => 'Base price must be a valid amount in pence.',
            'base_price.min' => 'Base price cannot be negative.',
            'duration_minutes.min' => 'Service duration must be at least 15 minutes.',
            'duration_minutes.max' => 'Service duration cannot exceed 8 hours.',
            'vendor_id.exists' => 'Selected vendor does not exist.',
            'consultation_duration_minutes.min' => 'Consultation duration must be at least 15 minutes.',
            'consultation_duration_minutes.max' => 'Consultation duration cannot exceed 2 hours.',
            'images.*.image' => 'Each file must be an image.',
            'images.*.mimes' => 'Images must be in JPEG, PNG, or WebP format.',
            'images.*.max' => 'Each image must not exceed 5MB.',
        ];
    }

    /**
     * Configure the validator instance.
     */
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            $service = $this->route('service');

            // If requires_consultation is being set to true, consultation_duration_minutes should be provided
            if ($this->requires_consultation && !$this->consultation_duration_minutes && !$service->consultation_duration_minutes) {
                $validator->errors()->add('consultation_duration_minutes', 'Consultation duration is required when consultation is enabled.');
            }

            // Validate vendor assignment permissions
            if ($this->has('vendor_id') && !$this->user()->hasPermission('manage_all_services')) {
                $validator->errors()->add('vendor_id', 'You do not have permission to change vendor assignment.');
            }

            // Check if service has active bookings when trying to deactivate
            if ($this->has('is_active') && !$this->is_active) {
                $activeBookingsCount = $service->bookings()
                    ->whereIn('status', ['pending', 'confirmed', 'in_progress'])
                    ->count();

                if ($activeBookingsCount > 0) {
                    $validator->errors()->add('is_active', "Cannot deactivate service with {$activeBookingsCount} active bookings.");
                }
            }

            // Check if service has active bookings when trying to make it non-bookable
            if ($this->has('is_bookable') && !$this->is_bookable) {
                $futureBookingsCount = $service->bookings()
                    ->whereIn('status', ['pending', 'confirmed'])
                    ->where('scheduled_at', '>', now())
                    ->count();

                if ($futureBookingsCount > 0) {
                    $validator->errors()->add('is_bookable', "Cannot make service non-bookable with {$futureBookingsCount} future bookings.");
                }
            }

            // Validate advance booking constraints
            $minHours = $this->min_advance_booking_hours ?? $service->min_advance_booking_hours;
            $maxDays = $this->max_advance_booking_days ?? $service->max_advance_booking_days;

            if ($minHours && $maxDays) {
                $maxHours = $maxDays * 24;
                if ($minHours > $maxHours) {
                    $validator->errors()->add('min_advance_booking_hours', 'Minimum advance booking cannot exceed maximum advance booking period.');
                }
            }
        });
    }

    /**
     * Prepare the data for validation.
     */
    protected function prepareForValidation(): void
    {
        // Convert price from pounds to pence if needed
        if ($this->has('base_price') && is_numeric($this->base_price)) {
            // If the price looks like it's in pounds (has decimal), convert to pence
            if (str_contains($this->base_price, '.') || $this->base_price < 100) {
                $this->merge([
                    'base_price' => (int) round($this->base_price * 100)
                ]);
            }
        }

        // Convert boolean fields
        if ($this->has('is_active')) {
            $this->merge(['is_active' => $this->boolean('is_active')]);
        }
        if ($this->has('is_bookable')) {
            $this->merge(['is_bookable' => $this->boolean('is_bookable')]);
        }
        if ($this->has('requires_consultation')) {
            $this->merge(['requires_consultation' => $this->boolean('requires_consultation')]);
        }
        if ($this->has('replace_images')) {
            $this->merge(['replace_images' => $this->boolean('replace_images')]);
        }
    }
}

...
class UpdateShippingRateRequest extends BaseFormRequest
{
    ...
    {
        return [
            'shipping_method_id' => ['sometimes', 'required', 'integer', 'exists:shipping_methods,id'],
            'shipping_zone_id' => ['sometimes', 'required', 'integer', 'exists:shipping_zones,id'],
            'min_weight' => ['sometimes', 'required', 'numeric', 'min:0', 'max:999999.99'],
            'max_weight' => ['nullable', 'numeric', 'min:0', 'max:999999.99', 'gte:min_weight'],
            'min_total' => ['sometimes', 'required', 'numeric', 'min:0'],
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            $currentRateId = $this->route('shippingRate')?->id;
            $methodId = $this->input('shipping_method_id') ?? $this->route('shippingRate')?->shipping_method_id;
            $zoneId = $this->input('shipping_zone_id') ?? $this->route('shippingRate')?->shipping_zone_id;
            $minWeight = $this->input('min_weight') ?? $this->route('shippingRate')?->min_weight;
            $maxWeight = $this->has('max_weight') ? $this->input('max_weight') : $this->route('shippingRate')?->max_weight;
            $minTotal = $this->input('min_total') ?? $this->route('shippingRate')?->min_total;
            $maxTotal = $this->has('max_total') ? $this->input('max_total') : $this->route('shippingRate')?->max_total;

            if ($methodId && $zoneId && $minWeight !== null && $minTotal !== null) {

...
class FilterProductRequest extends BaseFormRequest
{
    ...
    {
        return [
            'query' => $this->input('filter.search', ''),
    ...
    public function getFilters(): array
    {
        ...
        unset($filters['search'], $filters['include']);
    ...
    {
        return [
            'diversify' => $this->boolean('diversify', false),
            'explain' => $this->boolean('explain', false),
            'facets' => $this->boolean('facets', true),
            'highlight' => $this->boolean('highlight', false),
            'personalize' => $this->boolean('personalize', true),
            'typo_tolerance' => $this->boolean('typo_tolerance', true),
            'exact_match' => $this->boolean('exact_match', false),
            'boost_new_products' => $this->boolean('boost_new_products', true),
            'boost_in_stock' => $this->boolean('boost_in_stock', true),
        ];
    }

    public function getIncludes(): array
    {
        $include = $this->input('filter.include', '');
        if (empty($include)) {
            return ['vendor', 'category', 'productStatus', 'media'];
        }
    ...
    {
        return !empty($this->input('filter.search'));
    }

    public function shouldIncludeFacets(): bool
    {
        return $this->boolean('facets', true);
    }

    public function shouldExplainResults(): bool
    {
        return $this->boolean('explain', false) || config('search.debug.explain_scores', false);
    }
    ...
    public function getComplexityScore(): int
    {
        ...
        $searchQuery = $this->input('filter.search', '');
        $score += strlen($searchQuery) / 10;
        $score += substr_count($searchQuery, ' ');

        $filters = $this->getFilters();
        $score += count($filters) * 2;

        $includes = $this->getIncludes();
        $score += count($includes);

        $options = $this->getSearchOptions();
        if ($options['diversify']) $score += 5;
        if ($options['explain']) $score += 3;
        if ($options['personalize']) $score += 4;
